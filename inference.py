...
STRATEGY = "Anomaly_FedAVG"
ACCUMULATED_RESULTS = "./results/Experiments_2.3_Results/2.3_uk25_Anomaly_FedAVG_Results.csv"
...
result_sources = {
    'components': [41, 40, 39, 37, 36, 35, 34, 33, 32, 29, 26, 23, 20],
    'folds': [1, 2, 3, 4, 5],
    'marker': ['o', '-', '^' 'x', '-o-'],
    'clients': [1, 2, 3, 4],
    'path': './results/Experiments_2.3_Results/original_{0}_fold_{1}'
}
    ...
    for component in components:  
        for fold in folds:
            ...
            for client in clients:                                
                ...
                threshold = 0.20
                # fpr, tpr, thresholds_roc = roc_curve(labels, reconstruction_errors)
                # optimal_idx = np.argmax(tpr - fpr)  # Choose threshold where TPR - FPR is maximized
                # threshold = thresholds_roc[optimal_idx]
                
                # thresholds = np.linspace(0, max(reconstruction_errors), 100)
                # best_f1_score = 0
                # best_threshold = 0
                # for th in thresholds:
                #     preds = (np.array(reconstruction_errors) > th).astype(int)
                ...
                #mean_error = np.mean(benign_errors)
                #std_error = np.std(benign_errors)
                #threshold = mean_error + 1 * std_error  # for example, 3 standard deviations above the mean
...
if __name__ == "__main__":
    ...
    result_df, store_results_df = accumulate_results(result_sources, confusion_matrix_data, 'global_model.pth')
